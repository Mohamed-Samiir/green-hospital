...
export interface TodayTripsTableItem {
  id: number;
  name: string;
  status: boolean;
  rating: number;
...
const EXAMPLE_DATA: TodayTripsTableItem[] = [
 ...
 */
export class PassangersTableDataSource extends DataSource<TodayTripsTableItem> {
  data: TodayTripsTableItem[] = EXAMPLE_DATA;
   ...
   */
  connect(): Observable<TodayTripsTableItem[]> {
   ...
   */
  private getPagedData(data: TodayTripsTableItem[]): TodayTripsTableItem[] {
    if (this.paginator) {
   ...
   * this would be replaced by requesting the appropriate data from the server.
   */
  private getSortedData(data: TodayTripsTableItem[]): TodayTripsTableItem[] {
